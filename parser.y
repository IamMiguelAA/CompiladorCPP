...
Declaracion:  INT ID  Declespe  { if(si==1 || sino==1){/*variablesenteras.push_back(varint($2));*/ variables[*$1]=0;}}
		...
		|INT ID '=' {DecOrExp=1;} exp {if(si==1 || sino==1){/*variablesenteras.push_back(varint($2,$5));*/ variables[*$2]=$5; if(enfuncion==1){auto id=new NodoId(*$1); id->nuevaasign(fichero,contadorlocales); locales[*$2]=contadorlocales;fichero<<endl; contadorlocales++;}}}
		|INT ID '[' NUM ']' 
...
term: term {multi.escribepush(fichero); fichero<<endl; } '*' fact {$$=$1*$4; multi=*(new NodoMul($1,$4)); multi.escribe(fichero); fichero<<endl;}
	...
	|fact {$$=$1;}
;
fact: NUM {$$=$1; if(enllamada==1){call.insertarnum(llamada,$1);}else if(enfuncion==1 && retu!=1 ){auto num=new NodoNum($1); num->escribe(fichero); fichero<<endl;}}
	|'-' NUM {$$=-$2; if(enllamada==1){call.insertarnum(llamada,-$2);} else if(enfuncion==1 && retu!=1 ){auto num=new NodoNum(-$2); num->escribe(fichero); fichero<<endl;}}
	|'(' exp ')' {$$=$2;}
...
}else if(enfuncion==1){if(locales.find(*$1)!=locales.end()){int aux=buscarenmap(*$1,locales);auto id=new NodoId(*$1); id->escribe(fichero,1,aux); fichero<<endl;}else if(parametros.find(*$1)!=parametros.end()){int aux=buscarenmap(*$1,parametros);auto id=new NodoId(*$1); id->escribe(fichero,2,aux); fichero<<endl;}}
}
